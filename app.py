...
def normalizar_nombre_empresa(nombre, mapeo):
    ...
    # Usa el mapeo para obtener el nombre estandarizado; si no existe, devuelve el nombre ya limpiado
...
def render_analisis_comparativo(df_original):
    st.header("Análisis Comparativo por Turno")
    
    st.sidebar.header("Filtros para Comparación por Turno")
    ...
    # Selectores para Período 1 ("Turno 39")
    st.sidebar.markdown("#### Turno 39")
    fecha_inicio_1 = st.sidebar.date_input("Fecha Inicio Turno 39:", value=fechas_comp[-7] if len(fechas_comp) > 7 else fechas_comp[0], key="start1")
    fecha_fin_1 = st.sidebar.date_input("Fecha Fin Turno 39:", value=fechas_comp[-1], key="end1")
    
    # Selectores para Período 2 ("Turno 44")
    st.sidebar.markdown("#### Turno 44")
    fecha_inicio_2 = st.sidebar.date_input("Fecha Inicio Turno 44:", value=fechas_comp[-14] if len(fechas_comp) > 14 else fechas_comp[0], key="start2")
    fecha_fin_2 = st.sidebar.date_input("Fecha Fin Turno 44:", value=fechas_comp[-8] if len(fechas_comp) > 8 else fechas_comp[0], key="end2")
    ...
    st.metric("Variación de Tonelaje (Turno 39 vs 44)", f"{kpi1_total:,.2f} Ton", f"{delta_total:,.2f} Ton ({delta_total_percent:.1f}%)")
    ...
    def plot_comparison(group_by_col, df1, df2, title_suffix):
        ...
        col1, col2 = st.columns(2)
        with col1:
            st.markdown(f"**Turno 39** ({fecha_inicio_1.strftime('%d/%m')} - {fecha_fin_1.strftime('%d/%m')})")
            if not df1.empty:
        ...
        with col2:
            st.markdown(f"**Turno 44** ({fecha_inicio_2.strftime('%d/%m')} - {fecha_fin_2.strftime('%d/%m')})")
            if not df2.empty:
