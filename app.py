...
def normalizar_nombre_empresa(nombre):
    ...
    # Buscar el nombre limpio en las claves del diccionario. Si no se encuentra, devolver el nombre original normalizado.
...
def render_analisis_diario(df):
    ...
    if df_filtrado_op.empty:
        st.warning(f"No se encontraron datos para la fecha seleccionada: {fecha_sel_op.strftime('%d-%m-%Y')}")
        return
